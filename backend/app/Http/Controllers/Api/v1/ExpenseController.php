...
class ExpenseController extends Controller
{
    ...
    public function nextExpenseNumber(): \Illuminate\Http\JsonResponse
    {
        $number = $this->generateNextExpenseNumber();
        return response()->json([
            'status' => 'Success',
    ...
    private function generateNextExpenseNumber(): string
    {
        ...
        $prefix = $setting->expense_invoice_prefix ?? 'EXP-';
        $digit = max(1, (int)($setting->expense_invoice_digit ?? 4));
        $startFrom = (int)($setting->expense_invoice_start_from ?? 1);

        $last = Expense::where('invoice_number', 'like', $prefix . '%')
            ->orderByDesc('id')
            ->lockForUpdate()
            ->first();
    ...
    public function store(Request $request)
    {
        ...
        $setting = GeneralSetting::first();
        if ($setting && $setting->expense_invoice_enable_auto_generation && empty($validated['invoice_number'])) {
            $validated['invoice_number'] = $this->generateNextExpenseNumber();
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->hasFile('document')) {
            if ($expense->document) {
                \Illuminate\Support\Facades\Storage::disk('public')->delete($expense->document);
            }
            $path = $request->file('document')->store('expenses', 'public');
            $validated['document'] = $path;
        }
