...
export default function EmailSmsLogPage() {
    ...
    const fetchLogs = async (page: number = currentPage) => {
        setLoading(true);
        try {
            const response = await api.get(`/communicate/logs?page=${page}&per_page=${perPage}`);
    ...
    const handleDeleteSelected = async () => {
        try {
            ...
            tt.success("logs_deleted", { count: toLocaleNumber(selectedIds.size, language?.short_code) });
        ...
        <div className="p-4 space-y-4 bg-gray-50/10 min-h-screen font-sans">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] border border-gray-100 rounded-lg shadow-sm overflow-hidden">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <h1 className="text-[15px] font-bold text-gray-800 tracking-tight leading-none">{t("email_sms_wa_logs") || "Email / SMS / WhatsApp Logs"}</h1>
                        <p className="text-[11px] text-gray-500 mt-1">{t("sent_message_history") || "Sent Message History"}</p>
                    </div>
                </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-4 space-y-4">
                ...
                <div className="space-y-2">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-gray-300">
                            ...
                            <p className="text-sm font-bold text-gray-400 uppercase tracking-tight mt-4">{t("no_logs_found") || "No Logs Found"}</p>
                            <p className="text-[10px] text-gray-400 mt-1">{t("no_logs_recorded_yet") || "No logs recorded yet"}</p>
                        </div>
                    ) : (
                ...
                {total <= perPage && total > 0 && (
                    <div className="flex items-center text-[11px] text-gray-400 font-medium pt-2 border-t border-gray-50">
                        {t("showing_all_entries", { total: toLocaleNumber(total, language?.short_code) }) || `Showing all ${total} entries`}
                    </div>
                )}
            </div>
                        ...
                        <div className="p-8 space-y-6 bg-white overflow-y-auto max-h-[65vh]">
                            <div className="flex flex-wrap items-center gap-2">
                                ...
                                {viewLog.hasAttachment && viewLog.attachment && (
                                    <button
                                        onClick={async () => {
                                            const numericId = parseInt(viewLog.id.replace('sms_', '').replace('email_', '').replace('wa_', ''), 10);
                                            try {
            ...
            <AlertDialog open={deleteConfirm} onOpenChange={setDeleteConfirm}>
                <AlertDialogContent className="rounded-lg border-0 shadow-2xl">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-xl font-bold text-gray-800">{t("delete_selected_logs") || "Delete Selected Logs"}</AlertDialogTitle>
                        <AlertDialogDescription className="text-sm text-gray-500 leading-relaxed mt-2">
                            {t("delete_selected_logs_confirm", { count: toLocaleNumber(selectedIds.size, language?.short_code) }) || `Are you sure you want to delete ${selectedIds.size} selected logs?`}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
            ...
            <AlertDialog open={deleteAllConfirm} onOpenChange={setDeleteAllConfirm}>
                <AlertDialogContent className="rounded-lg border-0 shadow-2xl">
                    <AlertDialogHeader>
                        <AlertDialogTitle className="text-xl font-bold text-gray-800 text-red-500">{t("clear_all_logs") || "Clear All Logs"}</AlertDialogTitle>
                        <AlertDialogDescription className="text-sm text-gray-500 leading-relaxed mt-2">
                            {t("clear_all_logs_confirm") || "Are you sure you want to clear all logs? This action cannot be undone."}
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter className="mt-6">
                        <AlertDialogCancel className="h-10 rounded-lg text-[10px] font-bold uppercase tracking-wider border-gray-200">{t("cancel") || "Cancel"}</AlertDialogCancel>
