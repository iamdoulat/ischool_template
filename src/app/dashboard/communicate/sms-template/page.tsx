...
export default function SMSTemplatePage() {
    ...
    const handleDelete = async () => {
        ...
        setDeleting(true);
        try {
            await api.delete(`/communicate/sms-templates/${deleteId}`);
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, t("sms_templates") || "SMS Templates");
    ...
    const handleExportPDF = () => {
        const doc = new jsPDF();
        doc.text(t("sms_templates_report") || "SMS Templates Report", 14, 15);
    ...
    return (
        <div className="p-4 space-y-5 bg-gray-50/10 min-h-screen font-sans">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] border border-gray-100 rounded-xl shadow-sm overflow-hidden">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-base font-bold text-gray-800 tracking-tight leading-none">
                            {t("sms_template_list") || "SMS Templates"}
                        </h1>
                        <p className="text-[11px] text-gray-500 mt-1">
                            {t("sms_template_subtitle") || "Manage pre-defined SMS templates with character counter and dynamic tags"}
                        </p>
                    </div>
                </div>
                <Button
                ...
                >
                    <Plus className="h-4 w-4" /> {t("add_sms_template") || "Add SMS Template"}
                </Button>
            </div>
                                            ...
                                            <td colSpan={4} className="text-center py-14">
                                                <div className="flex flex-col items-center justify-center space-y-2 text-gray-400">
                                                    ...
                                                    <p className="text-xs font-semibold text-gray-600">
                                                        {t("no_sms_templates_found") || "No SMS templates found. Click \"Add SMS Template\" to create one."}
                                                    </p>
                                                </div>
                                            </td>
                                        </tr>
                    ...
                    <div className="bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] p-6 border-b border-gray-100">
                        <DialogHeader className="p-0">
                            <DialogTitle className="text-base font-bold tracking-tight text-slate-800 flex items-center gap-2">
                                ...
                                {viewMode ? (t("view_sms_template") || "View SMS Template") : editMode ? (t("edit_sms_template") || "Edit SMS Template") : (t("add_sms_template") || "Add SMS Template")}
                            </DialogTitle>
                            <DialogDescription className="text-xs text-gray-500 mt-1">
                                {viewMode ? (t("review_template_details") || "Review template details and formatted message body") : editMode ? (t("update_existing_sms_template") || "Update existing SMS template and variables") : (t("create_new_sms_template") || "Create new SMS template with dynamic variables and character limits")}
                            </DialogDescription>
                        </DialogHeader>
                    </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            {viewMode ? (
                                ...
                                <Input
                                    ...
                                    placeholder={t("sms_template_title_placeholder") || "Enter SMS template title..."}
                        ...
                        <div className="space-y-1.5">
                            ...
                            {viewMode ? (
                                ...
                                <Input
                                    ...
                                    placeholder={t("sms_template_id_placeholder") || "e.g. DLT-TE-01"}
                                ...
                                <>
                                    <div className="relative">
                                        <Textarea
                                            ...
                                            placeholder={t("enter_template_message_content") || "Write SMS message text here..."}
                                    ...
                                    <div className="flex items-center gap-2 text-[11px] font-medium pt-1">
                                        ...
                                        <span className={cn(
                                            "transition-colors",
                                            smsInfo.chars === 0 ? "text-gray-400" :
                                            smsInfo.segments <= 1 ? "text-emerald-600" :
                                            smsInfo.segments <= 3 ? "text-amber-600" :
                                            "text-rose-600"
                                        )}>
                                            {toLocaleNumber(smsInfo.chars, language?.short_code)} {t("characters") || "Characters"} ({toLocaleNumber(smsInfo.segments, language?.short_code)} {t("sms") || "SMS"})
                                            {smsInfo.chars > 0 && (
                                                <span className="text-gray-400 ml-1">
                                                    — {toLocaleNumber(smsInfo.perSegment, language?.short_code)} {t("per_segment") || "per segment"}{smsInfo.isUnicode ? ` (${t("unicode") || "Unicode"})` : ''}
                                                </span>
                                            )}
                                        </span>
                                    </div>
