...
export default function PostalReceivePage() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.from_title) {
            tt.error("from_title_required" || "From Title is required");
            return;
        }
        setSaving(true);
        try {
    ...
    const toggleSelectAll = () => {
        const displayedIds = displayedReceives.map(d => d.id);
    ...
    const handleCopy = () => {
        ...
        const text = ["From Title\tReference No\tTo Title\tDate", ...receives.map(d => `${d.from_title}\t${d.reference_no || "-"}\t${d.to_title || "-"}\t${d.date || "-"}`)].join("\n");
    ...
    const handleExportExcel = () => {
        if (receives.length === 0) return;
        const worksheet = XLSX.utils.json_to_sheet(receives.map(d => ({
        ...
        XLSX.utils.book_append_sheet(workbook, worksheet, "Postal Receives");
        XLSX.writeFile(workbook, "postal_receives.xlsx");
        ...
        <div className="space-y-6 font-sans p-3 sm:p-5 bg-gray-50/10 min-h-screen pb-20 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] border border-gray-100 dark:border-gray-800 rounded-2xl shadow-sm overflow-hidden no-print">
                <div className="flex items-center gap-2.5">
                    ...
                    <div>
                        <h1 className="text-[15px] font-bold text-gray-800 dark:text-gray-100 tracking-tight leading-none">{t("postal_receive")}</h1>
                        <p className="text-[11px] text-gray-500 mt-1">{t("record_an_incoming_item") || "Manage and record incoming postal packages"}</p>
                    </div>
                </div>
                <Button
                    ...
                    onClick={() => { setPage(1); fetchReceives(); }}
                    disabled={loading}
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                {/* Left Column: Add Postal Receive Form (Compact 1/3 layout) */}
                            ...
                            <form onSubmit={handleSave} className="space-y-3.5">
                                <div className="space-y-1 group">
                                    <label className="text-[11.5px] font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wider ml-0.5 group-focus-within:text-indigo-600 transition-colors">
                                        {t("from_title")} <span className="text-destructive font-black">*</span>
                                    </label>
                                    <Input
                ...
                {/* Right Column: Postal Receive List Table (2/3 layout) */}
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
                                        {loading ? (
                                            ...
                                            <tr>
                                                <td colSpan={7} className="px-4 py-12 text-center text-xs font-semibold text-gray-400">{t("no_postal_receives_found")}</td>
                                            </tr>
                                        ) : (
                                            displayedReceives.map((item, idx) => (
                                                <tr key={item.id} className={cn(
                                                        ...
                                                        <div className="flex items-center gap-2.5">
                                                            <Avatar className="h-8 w-8 rounded-full border border-indigo-100 dark:border-indigo-900 shadow-2xs shrink-0">
                                                                <AvatarFallback className="bg-gradient-to-br from-[#FF9800]/10 to-[#6366F1]/10 text-indigo-700 font-bold text-[11px]">
                                                                    {item.from_title ? item.from_title.substring(0, 2).toUpperCase() : "PR"}
                                                                </AvatarFallback>
                                                            </Avatar>
                                                    ...
                                                    <Td className="text-right whitespace-nowrap">
                                                        <div className="flex justify-end gap-1.5">
                                                            <ActionBtn icon={Eye} className="bg-indigo-500 hover:bg-indigo-600" title={t("view")} onClick={() => { setSelectedReceive(item); setIsViewDialogOpen(true); }} />
