...
export default function OnlineAdmissionPage() {
    ...
    const removeFile = () => {
        setSettings(prev => ({ ...prev, admission_form_file: null, admission_form_file_name: "" }));
        if (fileInputRef.current) fileInputRef.current.value = "";
    };
    ...
    const formatText = (command: string, textareaRef: React.RefObject<HTMLTextAreaElement>) => {
        ...
        switch (command) {
            case 'bold': formattedText = `**${selectedText}**`; newStart += 2; newEnd += 2; break;
            case 'italic': formattedText = `*${selectedText}*`; newStart += 1; newEnd += 1; break;
            case 'underline': formattedText = `<u>${selectedText}</u>`; newStart += 3; newEnd += 3; break;
            case 'align-center': formattedText = `<center>${selectedText}</center>`; newStart += 8; newEnd += 8; break;
            case 'align-right': formattedText = `<div style="text-align: right">${selectedText}</div>`; newStart += 31; newEnd += 31; break;
            case 'list': formattedText = `• ${selectedText}`; newStart += 2; newEnd += 2; break;
            case 'list-ordered': formattedText = `1. ${selectedText}`; newStart += 3; newEnd += 3; break;
            default: return;
        }
    ...
    const handleExport = (type: 'copy' | 'csv' | 'pdf' | 'print') => {
        ...
        if (type === 'copy') {
            ...
            const a = document.createElement('a'); a.href = url; a.download = `online_admission_fields.csv`; a.click();
                                    ...
                                    <div className="flex-1 p-4 bg-white overflow-auto">
                                        <div className="border border-gray-100 rounded overflow-hidden">
                                            <Table>
                                                ...
                                                <TableBody>
                                                    {filteredFields.length > 0 ? filteredFields.map((item) => (
                                        ...
                                        <div className="flex items-center justify-between pt-4">
                                            ...
                                            <div className="flex items-center gap-1">
                                                ...
                                                <Button className="h-7 w-7 p-0 text-[10px] font-bold bg-gradient-to-r from-[#FF9800] to-[#6366F1] text-white border-none rounded-md shadow-sm">1</Button>
                                                <Button variant="outline" size="icon" className="h-7 w-7 text-gray-400 border-gray-200" disabled>
