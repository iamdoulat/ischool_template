...
export default function WhatsappMessagingPage() {
    ...
    const handleTestMessage = async () => {
        ...
        try {
            ...
            if (isSuccess) {
                setTestResult({ ok: true, message: res.data?.message || "Test message sent successfully" });
                toast.success(res.data?.message || "Test message sent successfully");
            } else {
                setTestResult({ ok: false, message: res.data?.message || "Test message failed" });
                toast.error(res.data?.message || "Test message failed");
            }
        } catch (err: unknown) {
            const msg = err && typeof err === "object" && "response" in err
