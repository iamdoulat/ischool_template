...
export default function CurrencyPage() {
    ...
    const translateCurrencyName = (name: string, shortCode?: string) => {
        if (!name) return "";
        const key = name.toLowerCase().trim().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '');
        const translated = t(key);
        if (translated && translated !== key) return translated;
        if (shortCode) {
            const scKey = `currency_${shortCode.toLowerCase()}`;
            const scTrans = t(scKey);
            if (scTrans && scTrans !== scKey) return scTrans;
        }
        return name;
    };
                                    ...
                                    {currencies.map((currency, idx) => (
                                        <TableRow key={currency.id} className="border-b border-gray-50 hover:bg-indigo-50/40 hover:shadow-sm hover:z-10 relative transition-all duration-300 cursor-pointer h-12">
                                            ...
                                            <TableCell className="py-2 px-4 text-[11px] text-gray-600 font-medium">
                                                {translateCurrencyName(currency.currency, currency.short_code)}
                                            </TableCell>
