...
export default function SystemFieldsPage() {
    ...
    const handleExport = (type: "copy" | "excel" | "pdf" | "print") => {
        exportData(type, {
            ...
            title: `System Fields (${activeTab})`,
            columns: ["Name", "Status"],
            rows: filtered.map((f) => [f.name, f.is_active ? "Enabled" : "Disabled"]),
        });
    };
            ...
            <Card className="pt-0 overflow-hidden">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] border-b border-gray-100">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <h1 className="text-[15px] font-bold text-gray-800 tracking-tight leading-none">{t("system_fields")}</h1>
                            <p className="text-[11px] text-gray-500 mt-1">{t("toggle_student_and_staff_profile_fields")}</p>
                        </div>
                    </div>
                    <Tabs
                        value={activeTab}
                    ...
                    >
                        <TabsList className="bg-white/60 h-9 p-1 rounded-lg">
                            ...
                            <TabsTrigger value="staff" className="rounded-md data-[state=active]:bg-white data-[state=active]:text-indigo-600 text-gray-500 text-[12px] font-medium px-4">{t("staff")}</TabsTrigger>
                        </TabsList>
                    </Tabs>
                </div>
