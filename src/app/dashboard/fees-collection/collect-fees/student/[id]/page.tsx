...
import { cn, toLocaleNumber, formatDate } from "@/lib/utils";
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start md:items-center justify-between">
                <Button
                    ...
                    onClick={() => {
                        ...
                        router.push(`${prefix}/dashboard/fees-collection/collect-fees?student_id=${id}`);
                    }}
                ...
                >
                    ...
                    {t("back_to_student_list") || "Back to Student List"}
                </Button>
            </div>
                            ...
                            <div className="space-y-2 text-center sm:text-left">
                                ...
                                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                                    <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 font-bold px-3 py-1 rounded-lg text-xs">
                                        {t("admission_no")}: {studentData.admission_no || "-"}
                                    </Badge>
                                    <Badge variant="outline" className="bg-emerald-50 text-emerald-600 border-emerald-200 font-bold px-3 py-1 rounded-lg text-xs">
                                        {getLocalizedClassName(studentData.schoolClass?.name)} ({getLocalizedSectionName(studentData.section?.name)})
                                    </Badge>
                                </div>
                            </div>
                        </div>
            ...
            <Card className="border-[0.5px] border-gray-300 shadow-[0_4px_24px_rgb(0,0,0,0.06)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0 rounded-xl">
                <CardHeader className="flex flex-row items-center justify-between space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] border-b border-gray-100">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div>
                            <CardTitle className="text-sm sm:text-base font-bold tracking-tight text-slate-800 leading-none">{t("student_fees_details") || "Student Fees Details"}</CardTitle>
                            <p className="text-[11px] text-gray-500 mt-1">{toLocaleNumber(dueFees.length, shortCode)} {t("fee_records") || "fee records"}</p>
                        </div>
                    </div>
                </CardHeader>
                    ...
                    <Table className="w-full">
                        <TableHeader>
                            <TableRow className="bg-muted/30 border-b border-muted/50 hover:bg-muted/30 text-xs">
                                <TableHead className="font-bold text-slate-800 py-4 pl-6 sm:pl-8">{t("fees_group")}</TableHead>
                                <TableHead className="font-bold text-slate-800 py-4">{t("fees_type")}</TableHead>
                                <TableHead className="font-bold text-slate-800 py-4">{t("fees_code") || "Fees Code"}</TableHead>
                                <TableHead className="font-bold text-slate-800 py-4">{t("due_date")}</TableHead>
                                <TableHead className="font-bold text-slate-800 py-4 text-right">{t("fine")} ({symbol})</TableHead>
                                <TableHead className="font-bold text-slate-800 py-4 text-right">{t("amount")} ({symbol})</TableHead>
                                <TableHead className="font-bold text-slate-800 py-4 text-center pr-6 sm:pr-8">{t("action")}</TableHead>
                            </TableRow>
                        </TableHeader>
                        <TableBody>
                            {loading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={7} className="py-20 text-center">
                                        <div className="space-y-3">
                                            ...
                                            <p className="text-muted-foreground text-sm font-medium">{t("no_fee_records_found_for_this_student") || "No fee records found for this student."}</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : (
                                         ...
                                         <TableRow key={fee.is_transport ? `t_${fee.id}` : `r_${fee.id}`} className="group border-b border-muted/50 last:border-none hover:bg-muted/10 transition-colors">
                                             <TableCell className="py-4 pl-6 sm:pl-8">
                                                 <Badge variant="outline" className="bg-primary/5 text-primary border-primary/20 font-bold px-2.5 py-0.5 rounded-lg whitespace-nowrap text-xs">
                                                     {fee.fee_master?.fee_group?.name || t("general") || "General"}
                                                 </Badge>
                                             </TableCell>
                                             ...
                                             <TableCell className="py-4">
                                                 <div className="flex items-center gap-1.5 text-xs font-medium text-slate-600">
                                                     ...
                                                     {fee.fee_master?.due_date ? new Date(fee.fee_master.due_date).toLocaleDateString('en-GB') : (t("no_date") || "No Date")}
                                                 </div>
                                             </TableCell>
                                             ...
                                             <TableCell className="py-4 text-right">
                                                 <div className="space-y-0.5">
                                                     ...
                                                     <p className="text-[10px] font-bold text-emerald-600 uppercase tracking-tight">
                                                         {t("paid")}: {symbol}{paid.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                     </p>
                                                     {isPartial && (
                                                         <p className="text-[10px] font-bold text-amber-600 uppercase tracking-tight">
                                                             {t("due")}: {symbol}{due.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                         </p>
                                                     )}
                                                 </div>
                                             </TableCell>
                                                 ...
                                                 {fee.pending_payment ? (
                                                     <div className="flex flex-col items-center gap-1">
                                                         <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-50 text-amber-800 font-bold text-[10px] uppercase tracking-wider border border-amber-300 shadow-sm">
                                                             <Clock className="h-3 w-3 animate-pulse text-amber-600" /> {t("pending_approval") || "Pending Approval"}
                                                         </div>
                                                     ...
                                                     <div className="flex flex-col items-center gap-1">
                                                         ...
                                                         >
                                                             <CreditCard className="h-3.5 w-3.5" /> {t("collect") || "Collect"}
                                                         </Button>
                                                         {isPartial && (
                ...
                <DialogContent className="sm:max-w-[560px] p-0 max-h-[90vh] flex flex-col overflow-hidden rounded-2xl border border-gray-100 shadow-2xl">
                        ...
                        <div className="flex items-center gap-3.5">
                            ...
                            <div className="min-w-0 flex-1">
                                <DialogTitle className="text-lg sm:text-xl font-bold tracking-tight leading-snug">
                                    {t("collect_fee") || "Collect Fee"}
                                </DialogTitle>
                                <DialogDescription className="text-white/90 text-xs font-medium truncate mt-0.5">
                                    {selectedFee?.fee_master.fee_group?.name || t("general") || "General"} — {selectedFee?.fee_master.fee_type?.name} ({studentData?.name || t("student") || "Student"})
                                </DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                    ...
                    <form onSubmit={handlePaymentSubmit} className="flex-1 overflow-hidden flex flex-col">
                        <div className="flex-1 overflow-y-auto px-6 py-5 sm:px-7 sm:py-6 space-y-4 sm:space-y-5">
                            ...
                            <div className="p-3 bg-gradient-to-r from-amber-50/70 to-indigo-50/70 border border-indigo-100 rounded-xl flex items-center justify-between text-xs">
                                <div className="flex items-center gap-2 text-gray-700">
                                    ...
                                    <span className="font-semibold text-gray-700">{t("net_payable_amount") || "Net Payable Amount"}:</span>
                                </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1.5">
                                    <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider">
                                        {t("amount_to_pay") || "Amount To Pay"} ({symbol}) <span className="text-red-500">*</span>
                                    </label>
                                ...
                                <div className="space-y-1.5">
                                    <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider">
                                        {t("payment_date") || "Payment Date"} <span className="text-red-500">*</span>
                                    </label>
                                    <div className="relative">
                            ...
                            <div className="space-y-1.5">
                                ...
                                >
                                    <SelectTrigger className="h-10 rounded-lg border-gray-200 bg-gray-50/60 text-xs font-semibold focus:ring-indigo-500">
                                        <SelectValue placeholder={t("select_payment_mode") || "Select Payment Mode"} />
                                    </SelectTrigger>
                                    <SelectContent className="max-h-56">
                                        {paymentModes.some(m => m.isOnline) && (
                                            <SelectGroup>
                                                <SelectLabel className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider">{t("active_online_gateways") || "Active Online Gateways"}</SelectLabel>
                                        ...
                                        <SelectGroup>
                                            <SelectLabel className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mt-1">{t("offline_collection") || "Offline Collection"}</SelectLabel>
                                            {paymentModes.filter(m => !m.isOnline).map((mode) => (
                                                <SelectItem key={mode.id} value={mode.id} className="text-xs py-2">
                                                    <div className="flex items-center gap-2">
                                ...
                                <div className="p-3.5 bg-indigo-50/70 border border-indigo-100 rounded-xl space-y-3 text-xs animate-in fade-in duration-200">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-1.5 text-indigo-900 font-bold text-[11px]">
                                            <Zap className="h-3.5 w-3.5 text-amber-500" />
                                            <span>{paymentData.payment_mode} {t("online_processing") || "Online Processing"}</span>
                                        </div>
                                        {paymentData.payment_mode === "UddoktaPay" && (
                                            ...
                                            >
                                                {initiatingOnline ? (
                                                    <><Loader2 className="h-3 w-3 mr-1 animate-spin" /> {t("launching") || "Launching..."}</>
                                                ) : (
                                                    <><ExternalLink className="h-3 w-3 mr-1" /> {t("pay_via_uddoktapay") || "Pay via UddoktaPay"}</>
                                                )}
                                            </Button>
                                        )}
                                    </div>
                                    ...
                                    <div className="space-y-1.5">
                                        <div className="flex items-center justify-between">
                                            <label className="text-[10px] font-bold text-gray-700 uppercase tracking-wider">
                                                {t("transaction_id_reference") || "Transaction ID / Reference"} <span className="text-red-500">*</span>
                                            </label>
                                            <span className={cn(
                                        ...
                                        <Input
                                            type="text"
                                            placeholder={t("transaction_id_placeholder") || "e.g. TRX99887766 or Invoice ID"}
                                            className={cn(
                                        ...
                                        {trxLength <= 6 ? (
                                            <p className="text-[10px] text-amber-600 font-medium flex items-center gap-1 pt-0.5">
                                                ...
                                                {t("min_6_chars_required") || "Please input more than 6 characters to enable payment"}
                                            </p>
                                        ) : (
                                            <p className="text-[10px] text-emerald-600 font-medium flex items-center gap-1 pt-0.5">
                                                <CheckCircle2 className="h-3.5 w-3.5 shrink-0" />
                                                {t("valid_transaction_id_pending_approval") || "Valid Transaction ID — Payment will be submitted as Pending Approval"}
                                            </p>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="space-y-1.5">
                                <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider">
                                    {t("payment_note_remarks") || "Payment Note / Remarks"}
                                </label>
                                <div className="relative">
                                    ...
                                    <textarea
                                        ...
                                        placeholder={t("add_receipt_note_placeholder") || "Add receipt number, memo, or payment instructions..."}
                        ...
                        <DialogFooter className="px-6 py-3.5 sm:px-7 sm:py-4 bg-gray-50 border-t border-gray-100 flex flex-col-reverse sm:flex-row items-center justify-end gap-2.5 shrink-0">
                            ...
                            >
                                {submitting ? (
                                    <><Loader2 className="h-4 w-4 animate-spin mr-1.5" /> {t("processing") || "Processing..."}</>
                                ) : (
                                    <>
                                        <CheckCircle2 className="h-4 w-4 mr-1.5" />
                                        {isOnlineMode ? (t("submit_for_approval") || "Submit for Approval") : (t("complete_payment") || "Complete Payment")}
                                    </>
                                )}
                            </Button>
                        </DialogFooter>
