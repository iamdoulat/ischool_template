...
export default function AddHomeworkPage() {
    ...
    const handleSave = async () => {
        try {
            ...
            toast({
                title: "Error",
                description: error.response?.data?.message || "Failed to save homework",
                variant: "destructive",
            });
        }
    };
                ...
                <Tabs defaultValue="upcoming" className="w-full">
                             ...
                             <TableBody>
                                {homeworks.map((item) => (
                                    <TableRow key={item.id} className="text-[11px] border-b border-gray-50 hover:bg-gray-50/30 transition-colors whitespace-nowrap">
                                        <TableCell className="py-3 text-gray-700 font-medium">{item.schoolClass?.name}</TableCell>
                                        ...
                                        <TableCell className="py-3 text-right">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <Button size="icon" variant="ghost" className="h-7 w-7 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 rounded-full">
                                                    <Pencil className="h-3.5 w-3.5" />
                                                </Button>
                                                <Button size="icon" variant="ghost" onClick={() => handleDelete(item.id)} className="h-7 w-7 bg-rose-500 hover:bg-rose-600 text-white rounded-full shadow-sm">
                    ...
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        <DialogContent className="sm:max-w-[600px]">
                            <DialogHeader>
                                <DialogTitle className="text-lg font-bold text-gray-800">Add Homework</DialogTitle>
                            </DialogHeader>
