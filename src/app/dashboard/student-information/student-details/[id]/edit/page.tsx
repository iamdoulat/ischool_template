...
export default function StudentEditPage() {
    ...
    const fetchAutoGeneratedIds = async (cId?: string, sId?: string) => {
        ...
        try {
            ...
            // Fetch existing students in this specific class & section to calculate section-isolated sequence
            let classSectionCount = 0;
            try {
                const studentsRes = await api.get(`/students?school_class_id=${classId}&section_id=${sectionId}&no_paginate=true`);
                const existingStudents = studentsRes.data.data?.data || studentsRes.data.data || [];
                classSectionCount = Array.isArray(existingStudents) ? existingStudents.length : 0;
            } catch (err) {
                console.error("Error fetching class-section student count:", err);
            }
            ...
            const rollData = rollRes.data?.data || {};
            const userData = userRes.data?.data || {};

            const admPrefix = admData.prefix || admData.admission_no_prefix || "ADM-{class}{section}";
            const admDigit = admData.digit || admData.admission_no_digit || 4;
            const admStart = (parseInt(String(admData.start_from || admData.start_no || 1)) || 1) + classSectionCount;

            const rollPrefix = rollData.prefix || rollData.roll_no_prefix || "RL-{class}{section}";
            const rollDigit = rollData.digit || rollData.roll_no_digit || 4;
            const rollStart = (parseInt(String(rollData.start_from || rollData.start_no || 100)) || 100) + classSectionCount;

            const userPrefix = userData.prefix || userData.username_prefix || "STD-{class}{section}";
            const userDigit = userData.digit || userData.username_digit || 4;
            const userStart = (parseInt(String(userData.start_from || userData.username_start_from || 100)) || 100) + classSectionCount;

            const parentPrefix = userData.parent_prefix || userData.parent_username_prefix || "PAR-{class}{section}";
            ...
            setFormData(prev => ({
                ...prev,
                admission_no: formattedAdm || prev.admission_no,
                roll_no: formattedRoll || prev.roll_no,
                username: formattedUser || prev.username,
                parent_username: formattedParent || prev.parent_username,
            }));
        } catch (error) {
            console.error("Error generating auto IDs:", error);
        }
    };

    const fetchRollNo = async (classId: string, sectionId: string) => {
        fetchAutoGeneratedIds(classId, sectionId);
    };

    const fetchUsername = async (cId?: string, sId?: string) => {
        fetchAutoGeneratedIds(cId, sId);
    };
    ...
    const fetchStudentData = async () => {
        try {
            ...
            if (rawRoll.includes("{class}") || rawRoll.includes("{section}")) {
                rawRoll = formatAutoIdentifier("RL-{class}{section}", 4, 100, className, sectionName);
            }
            if (rawUser.includes("{class}") || rawUser.includes("{section}")) {
                rawUser = formatAutoIdentifier("STD-{class}{section}", 4, 100, className, sectionName);
            }
            if (rawParent.includes("{class}") || rawParent.includes("{section}")) {
