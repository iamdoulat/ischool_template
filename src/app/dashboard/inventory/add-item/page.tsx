...
export default function AddItemPage() {
    ...
    const exportData = items.map(i => ({
        ...
        [t("category")]: (i.itemCategory || i.item_category)?.item_category || "—",
        [t("unit")]: i.unit || "—",
        [t("available_quantity")]: i.available_quantity ?? i.available_qty ?? 0
    }));
    ...
    const handleCopy = () => {
        ...
        const text = items.map(i => `${i.item_name}\t${(i.itemCategory || i.item_category)?.item_category || ""}\t${i.unit}\t${i.description || ""}`).join('\n');
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, t("items") || "Items");
    ...
    const handleExportPDF = () => {
        ...
        const doc = new jsPDF();
        doc.text(t("item_list"), 14, 15);
        autoTable(doc, {
            head: [[t("item"), t("description"), t("category"), t("unit"), t("available_quantity")]],
            body: items.map(i => [
                i.item_name || "—",
                i.description || "—",
                (i.itemCategory || i.item_category)?.item_category || "—",
                i.unit || "—",
                i.available_quantity ?? i.available_qty ?? 0
            ]),
            startY: 20,
        });
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="h-fit border-[0.5px] border-gray-300 dark:border-zinc-800 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] dark:from-zinc-900 dark:to-zinc-950 dark:border-b dark:border-zinc-800">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-none">{isEditing ? t("edit_item") : t("add_item")}</CardTitle>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{isEditing ? t("update_selected_item") : t("create_new_inventory_item")}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("unit")} <span className="text-red-500">*</span></Label>
                ...
                <Card className="lg:col-span-2 border-[0.5px] border-gray-300 dark:border-zinc-800 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] dark:from-zinc-900 dark:to-zinc-950 dark:border-b dark:border-zinc-800">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-none">{t("item_list")}</CardTitle>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{toLocaleNumber(totalCount, shortCode)} {t("items")}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                            ...
                            <Table className="min-w-[900px]">
                                <TableHeader className="bg-gray-50 text-xs uppercase">
                                    <TableRow className="hover:bg-transparent whitespace-nowrap">
                                        ...
                                        <TableHead className="font-semibold text-gray-600 text-right"><div className="flex items-center justify-end gap-1">{t("available_quantity")} <ArrowUpDown className="h-2.5 w-2.5 opacity-30" /></div></TableHead>
