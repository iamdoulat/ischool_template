...
export default function AddItemStockPage() {
    ...
    const exportData = stocks.map(s => ({
        [t("item")]: s.item?.item_name || "—",
        [t("category")]: (s.itemCategory || s.item_category)?.item_category || "—",
        [t("supplier")]: s.supplier?.item_supplier || "—",
        [t("store")]: s.store?.item_store || "—",
        [t("quantity")]: s.quantity,
        [t("purchase_price")]: s.purchase_price,
    ...
    const handleCopy = () => {
        ...
        const text = stocks.map(s => `${s.item?.item_name || ""}\t${(s.itemCategory || s.item_category)?.item_category || ""}\t${s.supplier?.item_supplier || ""}\t${s.store?.item_store || ""}\t${s.quantity}\t${s.purchase_price}\t${s.date}`).join('\n');
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, t("stock_entries") || "StockEntries");
        XLSX.writeFile(wb, "item_stocks.xlsx");
    ...
    const handleExportCSV = () => {
        ...
        link.setAttribute("download", "item_stocks.csv");
    ...
    const handleExportPDF = () => {
        ...
        const doc = new jsPDF();
        doc.text(t("item_stock_list"), 14, 15);
        autoTable(doc, {
            head: [[t("item"), t("category"), t("supplier"), t("store"), t("qty"), t("purchase_price"), t("date")]],
            body: stocks.map(s => [
                s.item?.item_name || "—",
                (s.itemCategory || s.item_category)?.item_category || "—",
                s.supplier?.item_supplier || "—",
                s.store?.item_store || "—",
                s.quantity,
                s.purchase_price,
                formatDate(s.date)
    ...
    const totalCount = pagination?.total ?? stocks.length;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="h-fit border-[0.5px] border-gray-300 dark:border-zinc-800 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] dark:from-zinc-900 dark:to-zinc-950 dark:border-b dark:border-zinc-800">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-none">{isEditing ? t("edit_item_stock") : t("add_item_stock")}</CardTitle>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{isEditing ? t("update_stock_entry") : t("record_new_stock_entry")}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("item_category")} <span className="text-red-500">*</span></Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("item")} <span className="text-red-500">*</span></Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("supplier")}</Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("store")}</Label>
                        ...
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("quantity")} <span className="text-red-500">*</span></Label>
                            <Input type="number" value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("purchase_price")} <span className="text-red-500">*</span></Label>
                            <Input value={formData.purchase_price} onChange={(e) => setFormData({ ...formData, purchase_price: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("date")}</Label>
                            <DatePicker value={formData.date} onChange={(val) => setFormData({ ...formData, date: val })} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("attach_document")}</Label>
                            <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-200 dark:border-zinc-800 rounded-lg p-5 flex flex-col items-center justify-center gap-2 cursor-pointer hover:bg-gray-50 dark:hover:bg-zinc-900 transition-colors">
                ...
                <Card className="lg:col-span-2 border-[0.5px] border-gray-300 dark:border-zinc-800 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] dark:from-zinc-900 dark:to-zinc-950 dark:border-b dark:border-zinc-800">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-none">{t("item_stock_list")}</CardTitle>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{toLocaleNumber(totalCount, shortCode)} {t("stock_entries")}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                                ...
                                <TableBody>
                                    {loading ? (
                                        ...
                                        <TableRow key={stock.id} className="text-xs hover:bg-indigo-50/40 hover:shadow-sm hover:z-10 relative transition-all duration-300 cursor-pointer whitespace-nowrap">
                                            ...
                                            <TableCell className="py-3 text-right font-bold text-indigo-600">{toLocaleNumber(stock.quantity, shortCode)}</TableCell>
                                            <TableCell className="py-3 text-right text-gray-500">{toLocaleNumber(stock.purchase_price, shortCode)}</TableCell>
                                            <TableCell className="py-3 text-gray-500">{toLocaleNumber(formatDate(stock.date), shortCode)}</TableCell>
