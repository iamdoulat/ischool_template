...
export default function ItemCategoryPage() {
    ...
    const handleSave = async () => {
        if (!formData.item_category) {
            toast({ title: "Validation Error", description: "Item Category is required", variant: "destructive" });
            return;
        }
        setSaving(true);
        try {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="h-fit border-[0.5px] border-gray-300 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD]">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 leading-none">{isEditing ? "Edit Item Category" : "Add Item Category"}</CardTitle>
                            <p className="text-[11px] text-gray-500 mt-1">{isEditing ? "Update selected category" : "Create a new category"}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <div className="flex justify-end pt-2 gap-2">
                            {isEditing && (
                                <Button variant="outline" onClick={resetForm} className="h-9 px-6 rounded-full text-xs font-bold">Cancel</Button>
                            )}
                ...
                <Card className="lg:col-span-2 border-[0.5px] border-gray-300 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD]">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 leading-none">Item Category List</CardTitle>
                            <p className="text-[11px] text-gray-500 mt-1">{pagination?.total ?? categories.length} categor{(pagination?.total ?? categories.length) === 1 ? "y" : "ies"}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                            ...
                            <Table className="min-w-[600px]">
                                <TableHeader className="bg-gray-50 text-xs uppercase">
                                    <TableRow className="hover:bg-transparent whitespace-nowrap">
                                        <TableHead className="font-semibold text-gray-600"><div className="flex items-center gap-1">Item Category <ArrowUpDown className="h-2.5 w-2.5 opacity-30" /></div></TableHead>
                                ...
                                <TableBody>
                                    {loading ? (
                                        ...
                                        <TableRow key={cat.id} className="text-xs hover:bg-gray-50/60 transition-colors">
                                            ...
                                            <TableCell className="py-3 text-gray-400 italic">{cat.description || "No description"}</TableCell>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent className="sm:max-w-[400px]">
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>Are you sure you want to delete this item category? This action cannot be undone.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
