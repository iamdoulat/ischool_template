...
export default function ItemStorePage() {
    ...
    const exportData = stores.map(s => ({
        [t("item_store_name")]: s.item_store || "—",
        [t("item_store_code")]: s.code || "—",
    ...
    const handleExportExcel = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, t("stores") || "Stores");
        XLSX.writeFile(wb, "item_stores.xlsx");
    ...
    const handleExportPDF = () => {
        ...
        const doc = new jsPDF();
        doc.text(t("item_store_list"), 14, 15);
        autoTable(doc, {
            head: [[t("item_store_name"), t("item_store_code"), t("description")]],
            body: stores.map(s => [
                s.item_store || "—",
                s.code || "—",
                s.description || "—"
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="h-fit border-[0.5px] border-gray-300 dark:border-zinc-800 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] dark:from-zinc-900 dark:to-zinc-950 dark:border-b dark:border-zinc-800">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-none">{isEditing ? t("edit_item_store") : t("add_item_store")}</CardTitle>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{isEditing ? t("update_selected_store") : t("create_new_store")}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("item_store_name")} <span className="text-red-500">*</span></Label>
                            <Input value={formData.item_store} onChange={(e) => setFormData({ ...formData, item_store: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600 dark:text-gray-300">{t("item_store_code")}</Label>
                ...
                <Card className="lg:col-span-2 border-[0.5px] border-gray-300 dark:border-zinc-800 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD] dark:from-zinc-900 dark:to-zinc-950 dark:border-b dark:border-zinc-800">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 dark:text-slate-100 leading-none">{t("item_store_list")}</CardTitle>
                            <p className="text-[11px] text-gray-500 dark:text-gray-400 mt-1">{toLocaleNumber(totalCount, shortCode)} {t("stores")}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                            ...
                            <Table className="min-w-[600px]">
                                <TableHeader className="bg-gray-50 text-xs uppercase">
                                    <TableRow className="hover:bg-transparent whitespace-nowrap">
                                        ...
                                        <TableHead className="font-semibold text-gray-600"><div className="flex items-center gap-1">{t("item_store_code")} <ArrowUpDown className="h-2.5 w-2.5 opacity-30" /></div></TableHead>
                                ...
                                <TableBody>
                                    {loading ? (
                                        ...
                                        <TableRow key={store.id} className="text-xs hover:bg-indigo-50/40 hover:shadow-sm hover:z-10 relative transition-all duration-300 cursor-pointer">
                                            ...
                                            <TableCell className="py-3 text-gray-500">{store.code ? toLocaleNumber(store.code, shortCode) : "—"}</TableCell>
