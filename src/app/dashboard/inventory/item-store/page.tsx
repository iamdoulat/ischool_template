...
export default function ItemStorePage() {
    ...
    const handleSave = async () => {
        if (!formData.item_store) {
            toast({ title: "Validation Error", description: "Item Store Name is required", variant: "destructive" });
            return;
        }
        setSaving(true);
        try {
        ...
        <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="h-fit border-[0.5px] border-gray-300 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD]">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 leading-none">{isEditing ? "Edit Item Store" : "Add Item Store"}</CardTitle>
                            <p className="text-[11px] text-gray-500 mt-1">{isEditing ? "Update selected store" : "Create a new store"}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600">Item Store Name <span className="text-red-500">*</span></Label>
                            <Input value={formData.item_store} onChange={(e) => setFormData({ ...formData, item_store: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                            <Label className="text-xs font-semibold text-gray-600">Item Store Code</Label>
                ...
                <Card className="lg:col-span-2 border-[0.5px] border-gray-300 shadow-[0_4px_24px_rgb(0,0,0,0.08)] bg-card/50 backdrop-blur-sm overflow-hidden pt-0">
                    <CardHeader className="flex flex-row items-center gap-2.5 space-y-0 px-5 py-4 bg-gradient-to-r from-[#FFF5E7] to-[#EFF0FD]">
                        ...
                        <div className="min-w-0">
                            <CardTitle className="text-base font-bold tracking-tight text-slate-800 leading-none">Item Store List</CardTitle>
                            <p className="text-[11px] text-gray-500 mt-1">{pagination?.total ?? stores.length} store{(pagination?.total ?? stores.length) === 1 ? "" : "s"}</p>
                        </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                            ...
                            <Table className="min-w-[600px]">
                                <TableHeader className="bg-gray-50 text-xs uppercase">
                                    <TableRow className="hover:bg-transparent whitespace-nowrap">
                                        <TableHead className="font-semibold text-gray-600"><div className="flex items-center gap-1">Item Store Name <ArrowUpDown className="h-2.5 w-2.5 opacity-30" /></div></TableHead>
                                        <TableHead className="font-semibold text-gray-600"><div className="flex items-center gap-1">Code <ArrowUpDown className="h-2.5 w-2.5 opacity-30" /></div></TableHead>
            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent className="sm:max-w-[400px]">
                    <AlertDialogHeader>
                        <AlertDialogTitle>Delete Item Store</AlertDialogTitle>
                        <AlertDialogDescription>Are you sure you want to delete this item store? This action cannot be undone.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
