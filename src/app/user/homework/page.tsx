...
const submissionStatusConfig: Record<string, { key: string; className: string }> = {
    pending:   { key: "not_submitted_status", className: "bg-amber-100 text-amber-700 border-amber-200" },
    submitted: { key: "submitted_status",     className: "bg-blue-100 text-blue-700 border-blue-200" },
    evaluated: { key: "evaluated_status",     className: "bg-emerald-100 text-emerald-700 border-emerald-200" },
};
...
export default function UserHomeworkPage() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setShowSubmitForm(false);
            toast({ title: t("success"), description: t("homework_submitted_successfully") });
            fetchHomework();
        } catch (err: unknown) {
        ...
        if (status === "Evaluated") {
            return (
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                    ...
                    {t("evaluated_status")} {marks !== null && marks !== undefined ? `(${toLocaleNumber(marks, langCode)}${maxMarks ? `/${toLocaleNumber(maxMarks, langCode)}` : ""})` : ""}
                </span>
            );
        }
        return (
    ...
    const submissionBadge = (sub: Submission | null) => {
        const cfg = submissionStatusConfig[sub?.status || "pending"] || submissionStatusConfig.pending;
        return (
                    ...
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="min-w-0 space-y-0.5">
                            ...
                            <p className="text-[11px] text-gray-500 dark:text-gray-400">
                                {loading ? t("loading") : t(activeTab === "upcoming" ? "upcoming_assignments" : "closed_assignments", {
                ...
                <div className="flex border-b border-gray-200 dark:border-gray-800 px-3 sm:px-4 print:hidden bg-gray-50/40 dark:bg-gray-900">
                    {(["upcoming", "closed"] as const).map((tab) => (
                        <button
                            ...
                            className={cn(
                                "px-4 sm:px-5 py-3 text-[13px] font-semibold transition-colors border-b-2 -mb-px flex items-center gap-2",
                                activeTab === tab
                                    ? "border-[#6366F1] text-[#6366F1] font-bold"
                                    : "border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400"
                            )}
                        >
                            {tab === "upcoming" ? t("upcoming_homework") : t("closed_homework")}
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 print:hidden">
                        ...
                        <div className="flex items-center gap-2 flex-wrap w-full sm:w-auto justify-between sm:justify-end">
                            <Select value={String(limit)} onValueChange={(v) => { setLimit(Number(v)); setPage(1); }}>
                                ...
                                <SelectContent>
                                    {PAGE_SIZES.map((s) => <SelectItem key={s} value={String(s)}>{toLocaleNumber(s, langCode)}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        ...
                        <Table className="min-w-[960px]">
                            <TableHeader>
                                <TableRow className="bg-gray-50/90 dark:bg-gray-800/80 hover:bg-gray-50/90 border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <TableHead className="py-3 px-4 font-bold text-gray-700 dark:text-gray-300 text-center">{t("marks_label")}</TableHead>
                                ...
                                {loading ? (
                                    ...
                                    filtered.map((h) => (
                                        <TableRow key={h.id}
                                            ...
                                            <TableCell className="py-3 px-4 font-medium text-gray-800 dark:text-gray-200">{translateClassName(h.class, langCode)}</TableCell>
                                            <TableCell className="py-3 px-4">{h.section ? translateSectionName(h.section, langCode) : "—"}</TableCell>
                                            <TableCell className="py-3 px-4 font-semibold text-gray-800 dark:text-gray-200">{translateSubjectName(h.subject, langCode)}</TableCell>
                                            ...
                                            <TableCell className="py-3 px-4 text-center">{h.max_marks != null ? toLocaleNumber(h.max_marks, langCode) : "—"}</TableCell>
                                            <TableCell className="py-3 px-4 text-center font-bold text-emerald-600 dark:text-emerald-400">
                                                {h.marks_obtained != null ? toLocaleNumber(h.marks_obtained, langCode) : "—"}
                                            </TableCell>
                                ...
                                <button key={h.id} onClick={() => setSelected(h)}
                                    ...
                                    <div className="flex items-start justify-between gap-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                            ...
                                            <div className="min-w-0">
                                                <p className="text-[13.5px] font-bold text-gray-800 dark:text-gray-100 truncate">{translateSubjectName(h.subject, langCode)}</p>
                                                <p className="text-[11px] text-gray-500 dark:text-gray-400 flex items-center gap-1 mt-0.5">
                                                    <Layers className="h-3 w-3" />
                                                    {translateClassName(h.class, langCode)}{h.section ? ` · ${translateSectionName(h.section, langCode)}` : ""}
                                                </p>
                                            </div>
                                        </div>
                                        {statusBadge(h.status, h.marks_obtained, h.max_marks)}
                                    ...
                                    <div className="mt-3 grid grid-cols-2 gap-2 text-[11.5px] text-gray-500 dark:text-gray-400 bg-gray-50/80 dark:bg-gray-800/40 rounded-lg p-2.5 border border-gray-100 dark:border-gray-800">
                                        <span className="flex items-center gap-1.5">
                                            ...
                                            {t("homework_date")}: <span className="font-semibold text-gray-700 dark:text-gray-200">{fmt(h.homework_date)}</span>
                                        </span>
                                        <span className="flex items-center gap-1.5">
                                            <Calendar className="h-3.5 w-3.5 text-gray-400" />
                                            {t("submission_date")}: <span className="font-semibold text-gray-700 dark:text-gray-200">{fmt(h.submission_date)}</span>
                                        </span>
                                        <span className="flex items-center gap-1.5 col-span-2">
                                            <User className="h-3.5 w-3.5 text-gray-400" />
                                            {t("created_by")}: <span className="font-semibold text-gray-700 dark:text-gray-200">{h.created_by || "—"}</span>
                                        </span>
                                        {h.status === "Evaluated" && h.marks_obtained !== null && (
                                            <span className="flex items-center gap-1.5 col-span-2 text-emerald-700 dark:text-emerald-400 font-bold">
                                                ...
                                                {t("marks_obtained")}: {toLocaleNumber(h.marks_obtained, langCode)} {h.max_marks ? `/ ${toLocaleNumber(h.max_marks, langCode)}` : ""}
                                            </span>
                                        )}
                                    </div>
                            ...
                            <div className="flex items-center gap-3 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <h2 className="text-[15px] font-bold text-gray-800 dark:text-gray-100 leading-snug truncate">
                                        {selected.title || translateSubjectName(selected.subject, langCode)}
                                    </h2>
                                    <p className="text-[11px] text-gray-500 dark:text-gray-400">
                                        {translateSubjectName(selected.subject, langCode)} · {translateClassName(selected.class, langCode)}{selected.section ? ` · ${translateSectionName(selected.section, langCode)}` : ""}
                                    </p>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-2 gap-3 px-4 sm:px-5 pt-4 text-[12px] text-gray-500 dark:text-gray-400">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span>{t("homework_date")}: <span className="font-semibold text-gray-700 dark:text-gray-200">{fmt(selected.homework_date)}</span></span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <Calendar className="h-3.5 w-3.5 text-rose-500" />
                                    <span>{t("submission_date")}: <span className="font-semibold text-gray-700 dark:text-gray-200">{fmt(selected.submission_date)}</span></span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <ClipboardList className="h-3.5 w-3.5 text-amber-500" />
                                    <span>{t("max_marks") || "Max Marks"}: <span className="font-semibold text-gray-700 dark:text-gray-200">{selected.max_marks != null ? toLocaleNumber(selected.max_marks, langCode) : "—"}</span></span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <User className="h-3.5 w-3.5 text-sky-500" />
                                    <span>{t("created_by")}: <span className="font-semibold text-gray-700 dark:text-gray-200">{selected.created_by || "—"}</span></span>
                                </div>
                            </div>
                            ...
                            <div className="px-4 sm:px-5 pb-4 border-t border-gray-100 dark:border-gray-800 pt-4 space-y-3">
                                <div className="flex items-center justify-between">
                                    <p className="text-[12px] font-bold text-gray-700 dark:text-gray-200 uppercase tracking-wide">
                                        {t("evaluation_submission_status")}
                                    </p>
                                    {loadingSubmission ? (
                                    ...
                                    <div className="rounded-xl bg-emerald-50/80 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 p-3.5 space-y-2">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-2 text-emerald-800 dark:text-emerald-300">
                                                ...
                                                <span className="text-[14px] font-black">
                                                    {t("marks_label")}: {submission?.marks_obtained != null ? toLocaleNumber(submission.marks_obtained, langCode) : selected.marks_obtained != null ? toLocaleNumber(selected.marks_obtained, langCode) : "—"}
                                                    {selected.max_marks != null ? ` / ${toLocaleNumber(selected.max_marks, langCode)}` : ""}
                                                </span>
                                            </div>
                                        ...
                                        {(submission?.teacher_remarks || selected.teacher_remarks) && (
                                            <p className="text-[12px] text-emerald-800 dark:text-emerald-300 italic bg-white/70 dark:bg-gray-900/60 p-2 rounded-lg">
                                                {t("remarks")}: &ldquo;{submission?.teacher_remarks || selected.teacher_remarks}&rdquo;
                                            </p>
                                        )}
                                        {(submission?.evaluation_date || selected.evaluation_date) && (
                                            <p className="text-[11px] text-emerald-600 dark:text-emerald-400">
                                                {t("evaluated_on", { date: fmt(submission?.evaluation_date || selected.evaluation_date) })}
                                            </p>
                                        )}
                                    </div>
                                )}
                                ...
                                {submission?.status === "submitted" && selected.status !== "Evaluated" && (
                                    <div className="rounded-xl bg-blue-50/80 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 p-3.5 flex items-center gap-2.5 text-blue-700 dark:text-blue-300">
                                        ...
                                        <div>
                                            <p className="text-[12.5px] font-bold">{t("assignment_submitted")}</p>
                                            <p className="text-[11px] text-blue-600/80 dark:text-blue-400">
                                                {t("submitted_on_waiting", { date: submission.submitted_at ? fmt(submission.submitted_at.split("T")[0]) : "—" })}
                                            </p>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <>
                                        {!showSubmitForm ? (
                                            <Button onClick={() => setShowSubmitForm(true)}
                                                ...
                                                <Send className="h-3.5 w-3.5 mr-2" /> {t("submit_homework")}
                                            </Button>
                                        ) : (
                                            <div className="space-y-3 border border-indigo-100 dark:border-indigo-900 rounded-xl p-3.5 bg-indigo-50/30 dark:bg-indigo-950/20">
                                                <p className="text-[11px] font-bold text-indigo-700 dark:text-indigo-300 uppercase">{t("submit_your_work")}</p>
                                                <div className="space-y-1">
                                                    <Label className="text-[11px] font-bold text-gray-500 uppercase">{t("answer_notes")}</Label>
                                                    <Textarea
                                                        value={studentAnswer}
                                                        onChange={(e) => setStudentAnswer(e.target.value)}
                                                        className="text-[12px] min-h-[80px] border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800"
                                                        placeholder={t("write_answer_or_notes_here")} />
                                                </div>
                                                <div className="space-y-1">
                                                    <Label className="text-[11px] font-bold text-gray-500 uppercase">{t("attachment_optional")}</Label>
                                                ...
                                                <div className="flex gap-2">
                                                    <Button variant="outline" onClick={() => setShowSubmitForm(false)}
                                                        className="flex-1 h-8 text-[11px] rounded-[8px]">{t("cancel")}</Button>
                                                    <Button onClick={handleSubmit} disabled={submitting}
                                                        className="flex-1 h-8 text-[11px] rounded-[8px] bg-gradient-to-r from-[#FF9800] to-[#6366F1] text-white hover:opacity-90">
                                                        {submitting ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <><Send className="h-3.5 w-3.5 mr-1" /> {t("submit")}</>}
                                                    </Button>
                                                </div>
                                            </div>
                                        )}
